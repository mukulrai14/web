...
Prisma Compute is in [Public Beta](/console/more/feature-maturity#public-beta). This page lists what the current surface does and doesn't do, in one place.
...
- Resolution never reads or writes committed config files. `.prisma/local.json` is a gitignored local pin, and `PRISMA_PROJECT_ID` / `PRISMA_APP_ID` override it for CI.
...
- [Get started with `@prisma/cli`](/compute/getting-started): your first deploy, end to end.
