...
The CLI is the fastest way to get an app running on [Prisma Compute](/compute). This guide takes you from your code to a live URL, then shows the variations you'll reach for next. For every command, flag, and error code, see the [CLI reference](/compute/cli-reference).

The beta package is [`@prisma/cli`](https://github.com/prisma/prisma-cli). It installs an executable called `prisma-cli`.
...
That's it. If you're not signed in yet, the CLI walks you through it, then sets up the project, builds your app, and deploys it. When it finishes, you get a live URL.
...
Your first deployment is promoted to production automatically. After that, deploying to the production branch is an explicit choice: pass `--prod`:

```npm
npx @prisma/cli@latest app deploy --prod
```

Preview deploys never need it. See [Deployments](/compute/deployments) for promotion and rollback.
...
When a command fails in `--json` mode, branch on the error code rather than the message. The common ones:
...
- [Connect your app to Prisma Postgres](/compute/database/connect-to-prisma-postgres)
