...
metaImagePath: "/gremlin-turning-open-tasks-into-pull-requests/imgs/hero.svg"
tags: ["ai"]
---
...
A natural future direction is connecting Gremlin to products that already detect problems, rather than asking engineers to find them in a console.

[Query Insights](https://www.prisma.io/docs/query-insights) is an existing example. It is built into Prisma Postgres and already surfaces slow queries, expensive reads, and repeated statement shapes. Today it helps engineers understand what to change next: AI-generated analysis, copyable prompts, investigation in the Prisma Console.

That analysis step is useful, but it is still a separate surface. The engineer opens the console, reads the suggestion, switches to their editor, applies the fix, opens a PR. Gremlin closes that gap. Query Insights detects the issue; Gremlin receives the context and produces a pull request. The engineer's interface becomes the PR, not the console.

The same pattern applies elsewhere: detect a failure or degradation, evaluate whether it is in scope, hand it to Gremlin, review the result in the workflow teams already use. For Prisma Compute, Gremlin could operate as an always-on assistant for operational quality: watch for recurring issues, convert them into candidate tasks, and raise PRs before those issues become persistent sources of engineering noise.

Over time, with consistent results and explicit user approval, fixes in well-understood categories could merge automatically. That is a longer path. The near-term goal is reliable detection, sandboxed execution, and a reviewable PR, with selectivity at every step.

A proactive agent should not attempt everything. It should be conservative about what it chooses to fix, explicit about why a task is in scope, and transparent about its confidence. The more proactive the system becomes, the more important those boundaries become.
...
The current focus is small to medium-sized tasks. The longer-term direction is larger project execution, deeper product integration, and more proactive operational quality. The core principle remains the same: agents are most useful when they are embedded into real workflows, constrained by clear boundaries, and evaluated by the amount of friction they remove.
